 ...
 *  - OPTIONAL internal extension: when the neutral runtime
 *    `window.DSH_PLUGIN_INTEGRATION_V1` is present (loaded by any compatible
 *    plugin, e.g. dsh-lifecycle), it registers a `quota` module exposing
 *    `getSnapshot()`, `actions.refresh`, `mountCompact(container)`,
 *    `unmountCompact()` and `openDetail()`. This is an internal, user-invisible
 *    seam for a future "Whale Breath"-like layer; it never creates UI itself
 *    and never leaks quota internals (DOM classes) to the host. Standalone
 *    behaviour is unchanged when the runtime is absent.
 *
...
function buildPageScript(rows, refreshMs) {
  ...
  function anchorPanel() {
    ...
    var footer = document.querySelector('[aria-label="鲸息入口"]') || document.querySelector('[data-slot="settings.trigger"]');
    if (footer) {
      var footerRect = footer.getBoundingClientRect();
      if (footerRect.width > 0 && footerRect.left < window.innerWidth / 2) defaultPos.y = footerRect.top - 40;
    }
